    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          ...
          "${aws_s3_bucket.lambda_deployments_primary.arn}/*",
          aws_s3_bucket.lambda_deployments_dr.arn,
          "${aws_s3_bucket.lambda_deployments_dr.arn}/*",
          aws_s3_bucket.static_assets.arn,
          "${aws_s3_bucket.static_assets.arn}/*",
          aws_s3_bucket.static_assets_dr.arn,
          "${aws_s3_bucket.static_assets_dr.arn}/*"
        ]
      },
      {
...
output "cicd_setup_instructions" {
  ...
  value       = <<-EOT
    ...
    🔐 CI/CD User Created: ${aws_iam_user.cicd.name}
    
    To retrieve credentials for GitHub Actions:
    
    aws secretsmanager get-secret-value \
      --secret-id ${aws_secretsmanager_secret.cicd_credentials.name} \
    ...
    Then set GitHub secrets:
      ...
      gh secret set AWS_ACCOUNT_ID --body "${data.aws_caller_identity.current.account_id}" --repo apitanga/ssr-nuxt-nitro-poc
    
    ⚠️  IMPORTANT: These credentials grant access to deploy infrastructure.
        Rotate them regularly and monitor CloudTrail for unauthorized use.
