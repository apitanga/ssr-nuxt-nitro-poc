...
```
ssr-nuxt-nitro-poc/
├── Project Sketch - SSR Nuxt Nitro PoC.md    # Architecture document
├── 2026-01-30.md                              # This session note
├── terraform/
│   ├── main.tf
│   ├── variables.tf
│   ├── outputs.tf
...
## 📝 Terraform Notes & Issues

### Potential Issues to Watch
1. **Lambda module role_arn** - Currently creating separate roles; should reference shared role
2. **CloudFront origin domain** - Using `replace()` to strip `https://` from Function URLs; verify this works
3. **DynamoDB Global Table** - Using newer `aws_dynamodb_global_table` resource; may need `aws_dynamodb_table_replica` instead
...
### Deployment
1. [ ] Create S3 buckets for state backend
2. [ ] Run `terraform init`
3. [ ] Deploy to us-east-1 first (single region)
4. [ ] Test Lambda Function URL directly
5. [ ] Add CloudFront distribution
6. [ ] Add DR region and Global Tables
